...
check_and_install_packages() {
    local packages=("ufw" "fail2ban" "rsyslog")
    local to_install=()
    ...
    if [[ ${#to_install[@]} -gt 0 ]]; then
        ...
        # Redémarrer rsyslog si installé pour s'assurer que les logs fonctionnent
        if [[ " ${to_install[*]} " =~ " rsyslog " ]]; then
            systemctl restart rsyslog
            print_info "Service rsyslog redémarré"
        fi
    fi
    
    # Vérifier que rsyslog fonctionne
    if ! systemctl is-active --quiet rsyslog; then
        print_warning "Le service rsyslog n'est pas actif"
        systemctl start rsyslog || print_error "Impossible de démarrer rsyslog"
    fi
}
...
create_fail2ban_config() {
    ...
    print_info "Arrêt complet de Fail2ban..."
    systemctl stop fail2ban &>/dev/null || true
    sleep 2
    
    # Nettoyer complètement la configuration
    print_info "Nettoyage de la configuration existante..."
    ...
    # Supprimer tous les fichiers de configuration locaux
    rm -f /etc/fail2ban/jail.local
    rm -f /etc/fail2ban/jail.d/*.local 2>/dev/null || true
    
    # Détecter le fichier de log SSH approprié
    local ssh_logpath=""
    if [[ -f "/var/log/auth.log" ]] && [[ -s "/var/log/auth.log" ]]; then
        ...
        # Forcer la création du fichier auth.log si nécessaire
        print_info "Création du fichier de log auth.log..."
        touch /var/log/auth.log
        chown syslog:adm /var/log/auth.log 2>/dev/null || chown root:root /var/log/auth.log
        chmod 640 /var/log/auth.log
        
        # Redémarrer rsyslog pour prendre en compte le nouveau fichier
        systemctl restart rsyslog &>/dev/null || true
        ssh_logpath="/var/log/auth.log"
    fi
    
    print_info "Fichier de log SSH utilisé : $ssh_logpath"
    
    # Créer une configuration ultra-simple mais fonctionnelle
    print_info "Création d'une configuration ultra-simple..."
    cat > "$FAIL2BAN_LOCAL" << EOF
# Configuration Fail2ban ultra-simple
[DEFAULT]
bantime = 3600
findtime = 600
maxretry = 5
...
EOF
    ...
    print_success "Configuration ultra-simple créée"
    
    # Test de la configuration
    print_info "Test de la nouvelle configuration..."
    if fail2ban-client --test &>/dev/null; then
        print_success "Configuration validée"
    else
        print_error "Configuration toujours invalide - Réinstallation de Fail2ban nécessaire"
        
        read -p "Voulez-vous réinstaller complètement Fail2ban ? [Y/n]: " reinstall_f2b
        if [[ ! $reinstall_f2b =~ ^[Nn]$ ]]; then
            reinstall_fail2ban
            return $?
        else
            return 1
    ...
    print_info "Démarrage de Fail2ban..."
    systemctl enable fail2ban &>/dev/null
    
    if systemctl start fail2ban; then
        sleep 5
        if systemctl is-active --quiet fail2ban; then
            print_success "Fail2ban démarré avec succès !"
            
            # Vérifier les jails
            print_info "Vérification des jails actives..."
            local jail_status=$(timeout 10 fail2ban-client status 2>/dev/null || echo "timeout")
            if [[ "$jail_status" != "timeout" ]]; then
                echo "$jail_status"
            else
                print_warning "Délai d'attente pour le statut des jails"
            fi
        else
            print_error "Fail2ban ne démarre toujours pas"
            show_fail2ban_diagnostics
            return 1
        fi
    else
        print_error "Impossible de démarrer Fail2ban"
        show_fail2ban_diagnostics
        return 1
    fi
}
...
reinstall_fail2ban() {
    print_info "═══ RÉINSTALLATION COMPLÈTE DE FAIL2BAN ═══"
    
    # Arrêter le service
    systemctl stop fail2ban &>/dev/null || true
    systemctl disable fail2ban &>/dev/null || true
    
    # Purger complètement le paquet
    print_info "Suppression complète de Fail2ban..."
    apt purge -y fail2ban &>/dev/null
    ...
    print_info "Réinstallation de Fail2ban..."
    apt update &>/dev/null
    apt install -y fail2ban
    
    if [[ $? -eq 0 ]]; then
        print_success "Fail2ban réinstallé avec succès"
...
EOF
        ...
        print_info "Configuration minimale créée"
        ...
        if systemctl is-active --quiet fail2ban; then
            print_success "Fail2ban opérationnel après réinstallation"
            return 0
        else
            print_error "Problème persistant après réinstallation"
            return 1
        fi
    else
...
show_fail2ban_diagnostics() {
    ...
    echo
    echo "2. Vérification des fichiers de logs :"
    local log_files=("/var/log/auth.log" "/var/log/secure" "/var/log/messages" "/var/log/syslog")
    for log_file in "${log_files[@]}"; do
        if [[ -f "$log_file" ]]; then
            echo "   ✓ $log_file existe ($(stat -c%s "$log_file") octets)"
        else
    ...
    echo
    echo "3. Logs récents du service :"
    journalctl -u fail2ban --no-pager -n 8 --since "5 minutes ago" 2>/dev/null || echo "Pas de logs récents"
    
    echo
    echo "4. Filtres disponibles :"
    if [[ -d "/etc/fail2ban/filter.d" ]]; then
        echo "   Nombre de filtres : $(ls /etc/fail2ban/filter.d/*.conf 2>/dev/null | wc -l)"
        ls /etc/fail2ban/filter.d/*.conf 2>/dev/null | head -5 | xargs basename -s .conf | sed 's/^/   • /' || echo "   Aucun filtre trouvé"
    else
        echo "   Répertoire des filtres manquant"
    fi
    ...
    echo "   - Permissions : $(stat -c%a "$FAIL2BAN_LOCAL" 2>/dev/null || echo "inconnu")"
    
    echo
    echo "6. Processus Fail2ban :"
    pgrep -fl fail2ban || echo "   Aucun processus fail2ban en cours"
    
    # Proposer des solutions
    echo
    print_info "Solutions suggérées :"
    echo "  • Purger et réinstaller : apt purge fail2ban && apt install fail2ban"
    echo "  • Vérifier rsyslog : systemctl status rsyslog"
    echo "  • Logs en temps réel : tail -f /var/log/fail2ban.log"
    echo "  • Test manuel : fail2ban-client --test"
    echo "  • Configuration de base : cp /etc/fail2ban/jail.conf /etc/fail2ban/jail.local"
}
