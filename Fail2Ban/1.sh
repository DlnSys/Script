...
create_fail2ban_config() {
    ...
    if ! fail2ban-client --test &>/dev/null; then
        ...
        # Détecter le fichier de log SSH
        local ssh_logpath=""
        if [[ -f "/var/log/auth.log" ]]; then
            ...
            ssh_logpath="/var/log/auth.log"  # Par défaut
        fi
        
        print_info "Fichier de log SSH détecté : $ssh_logpath"
        
        # Configuration minimale fonctionnelle
        cat > "$FAIL2BAN_LOCAL" << EOF
# Configuration Fail2ban minimale générée automatiquement
[DEFAULT]
...
# Configuration email (si configuré)
EOF
...
EOF
            ...
            echo "action = %(action_)s" >> "$FAIL2BAN_LOCAL"
        fi
        
        cat >> "$FAIL2BAN_LOCAL" << EOF

# Jail SSH (seule jail activée par sécurité)
[sshd]
enabled = true
port = ssh
filter = sshd
