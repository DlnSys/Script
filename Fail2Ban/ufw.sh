...
create_fail2ban_config() {
    ...
    if fail2ban-client --test > /dev/null 2>&1; then
        ...
        print_warning "Erreur dans la configuration Fail2ban, vérification manuelle recommandée"
    fi
    ...
    if systemctl is-active --quiet fail2ban; then
        ...
        sleep 2
        if systemctl is-active --quiet fail2ban; then
            print_success "Service Fail2ban redémarré avec succès"
        else
            print_error "Erreur lors du redémarrage de Fail2ban"
            print_info "Vérifiez les logs: journalctl -u fail2ban -n 20"
        fi
    else
        ...
        sleep 2
        if systemctl is-active --quiet fail2ban; then
            print_success "Service Fail2ban activé et démarré"
        else
