...
configure_jails() {
    local jails=(
        "sshd:SSH:$SSH_PORT:10:3600:true"
        "apache:Apache:80,443:5:3600:false"
    ...
    for jail_info in "${jails[@]}"; do
        ...
        if [[ $activate_jail =~ ^[Yy]$ ]]; then
            ...
            if check_and_create_filters "$jail_name"; then
                ...
                read -p "Ports à surveiller [$default_ports]: " ports
                ...
                # Demander un logpath personnalisé seulement pour certains services
                local logpath=""
                if [[ "$jail_name" == "wordpress" ]] || [[ "$jail_name" == "webmin" ]] || [[ "$jail_name" == "named" ]] || [[ "$jail_name" == "ejabberd" ]]; then
                    read -p "Fichier de log personnalisé (laisser vide pour défaut): " logpath
                fi
                
                print_success "$service_name configuré: ports=$ports, maxretry=$maxretry, bantime=$bantime"
                
                # Stocker le logpath pour la configuration
                if [[ -n "$logpath" ]]; then
                    echo "LOGPATH_$jail_name=$logpath" >> /tmp/jail_logpaths.tmp
                fi
            else
...
bantime = $bantime"
            ...
            # Ajouter le logpath personnalisé si défini
            if [[ -f /tmp/jail_logpaths.tmp ]] && grep -q "LOGPATH_$jail_name=" /tmp/jail_logpaths.tmp; then
                local custom_logpath=$(grep "LOGPATH_$jail_name=" /tmp/jail_logpaths.tmp | cut -d'=' -f2-)
                FAIL2BAN_CONFIG="$FAIL2BAN_CONFIG
logpath = $custom_logpath"
            fi
        fi
...
create_fail2ban_config() {
    ...
    # Nettoyer le fichier temporaire des logpaths
    rm -f /tmp/jail_logpaths.tmp
    ...
    # Vérifier la syntaxe de la configuration avant de redémarrer
    print_info "Vérification de la syntaxe de la configuration..."
    if fail2ban-client --test &>/dev/null; then
        print_success "Configuration Fail2ban validée"
    else
        print_error "Erreur de syntaxe dans la configuration Fail2ban"
        print_info "Contenu de la configuration problématique :"
        echo "----------------------------------------"
        cat "$FAIL2BAN_LOCAL"
        echo "----------------------------------------"
        return 1
    ...
    if systemctl start fail2ban; then
        ...
        if systemctl is-active --quiet fail2ban; then
            ...
            fail2ban-client status 2>/dev/null || echo "Aucune jail active pour le moment"
        else
        ...
        print_info "Diagnostic des erreurs :"
        echo "1. Vérification de la syntaxe :"
        fail2ban-client --test 2>&1 | head -5
        echo "2. Logs du service :"
        ...
        if [[ -f "${FAIL2BAN_LOCAL}.backup."* ]]; then
            read -p "Restaurer la configuration précédente ? [y/N]: " restore_backup
            if [[ $restore_backup =~ ^[Yy]$ ]]; then
                local backup_file=$(ls -t "${FAIL2BAN_LOCAL}.backup."* | head -1)
                cp "$backup_file" "$FAIL2BAN_LOCAL"
                print_info "Configuration restaurée depuis : $backup_file"
